...
function coerceAttributeValue (spec, matchers, rawValue, attributeName) {
  ...
  if (R.is(Object)(matchers)) {
    if (!coerceAttributeValue.matcherFns) {
      coerceAttributeValue.matcherFns = {
        'number': transformNumber,
        'boolean': transformBoolean,
        'primitives': transformPrimitives,
        'collection': transformCollection,
    ...
    R.keys(matchers).some((m) => {
      ...
      const matchFn = coerceAttributeValue.matcherFns[R.toLower(m)];
      // console.log(`>>> coerceAttributeValue, matchFn: ${matchFn}, attributeName: ${attributeName}, rawValue: ${rawValue}`);
...
function transformBoolean (booleanValue, context, spec) {
  let value = R.toLower(booleanValue);
  let succeeded = false;

  if (R.either(R.equals('true'), R.equals('true'))(value)) {
    succeeded = true;
    value = Boolean(value);
  } else {
    value = booleanValue;
...
function transformString (stringValue, context, spec) {
  const bStringValue = toBoolean(stringValue);
  if (!bStringValue) {
...
function toBoolean (value) {
  if (R.either(R.equals('true'), R.equals('false'))(R.toLower(value))) {
