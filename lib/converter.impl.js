...
function getItem (itemLens, source) {
...
function validateSpec (spec) {
  ...
  // coercion validation
  //
  const coercion = getItem(R.lensProp('coercion'), spec);
  const attributes = getItem(R.lensProp('attributes'), coercion);

  // const optionalIs = (fn) => (o) => R.either(R.complement(R.prop), fn(o));
  // const optionalPropertyIs = R.either(R.complement(R.prop), R.__);

  // Validate attributes.trim
  //
  if (R.prop('trim')(attributes) &&
    R.where({
      trim: R.complement(R.is(Boolean))
    })(attributes)) {
    throw new Error(`Invalid coercion "trim" property, spec: ${functify(spec)}`);
  }

  // Validate attributes.matchers.primitives
  //
  R.forEach((p) => {
    if (R.not(R.includes(p, ['number', 'boolean', 'primitive']))) {
      throw new Error(`Invalid primitive value found: "${p}"`);
    }
  })(getItem(R.lensPath(['matchers', 'primitives']), attributes) || []);

  // Validate attributes.matchers.collection
  //
  const matchers = getItem(R.lensProp('matchers'), attributes);

  if (matchers) {
    const collectionMatcher = getItem(R.lensProp('collection'), matchers);

    // a: either(complement(x => x ? true : false), is(String))
    ...
    //   if (R.not(R.where({
    //     delim: R.either(R.complement(R.prop), R.is(String)),
    //     open: R.either(R.complement(R.prop), R.is(String)),
    //     close: R.either(R.complement(R.prop), R.is(String)),
    //     throwIfMatchFails: R.either(R.complement(R.prop), R.is(Boolean)),
    //     payload: R.either(R.complement(R.prop), R.is(Object))
    //   })(collectionMatcher))) {
    //     throw new Error(`Invalid collection matcher found: ${functify(collectionMatcher)}`);
    //   }
    // }

    // CRUDE-SOLUTION (WORKING)
    //
    // if (collectionMatcher) {
    //   if (R.not(R.where({
    //     delim: R.either(R.complement(x => x ? true : false), R.is(String)),
    //     open: R.either(R.complement(R.prop), R.is(String)),
    //     close: R.either(R.complement(R.prop), R.is(String)),
    //     throwIfMatchFails: R.either(R.complement(R.prop), R.is(Boolean)),
    //     payload: R.either(R.complement(R.prop), R.is(Object))
    ...
    // const optional = (fn) => (x) => R.either(R.complement(x => Boolean(x)), fn(x));

    // const optional = (fn) => (x) => R.either(R.complement(x => Boolean(x)), fn);

    const isUndefined = x => x === undefined;

    if (collectionMatcher) {
      if (R.not(R.where({
        delim: R.either(isUndefined, R.is(String)),

        // delim: R.either(R.complement(x => Boolean(x)), R.is(String)),

        // delim: R.either(R.not(x => Boolean(x)), R.is(String)),
        open: R.either(R.complement(R.prop), R.is(String)),
        close: R.either(R.complement(R.prop), R.is(String)),
        throwIfMatchFails: R.either(R.complement(R.prop), R.is(Boolean)),
        payload: R.either(R.complement(R.prop), R.is(Object))
    ...
    //
    const payload = getItem(R.lensProp('payload'), collectionMatcher);

    if (payload) {
      if (R.not(R.where({
        delim: R.either(R.complement(R.prop), R.is(String)),
        valuetype: R.either(R.complement(R.prop), R.is(String))
      })(payload))) {
