 ...
 */
function buildElement (elementNode, parentNode, spec, getOptions, previouslySeen = []) {
    ...
    if (recurseAttr !== '') {
      ...
      if (recurseAttributes.length > 1) {
        ...
        let inheritedElements = R.map(at => {
          ...
          //
          return buildElement(inheritedElementNode, parentNode, spec, getOptions,
            previouslySeen);
        }, recurseAttributes);
        ...
        //
        let inheritedElement = buildElement(inheritedElementNode, parentNode, spec, getOptions,
          previouslySeen);
...
function buildChildren (element, elementNode, spec, getOptions, previouslySeen) {
  ...
  if (selectionResult && selectionResult.length > 0) {
    ...
    let children = R.reduce((acc, childElement) => {
      let child = buildElement(childElement, elementNode, spec, getOptions, previouslySeen);
