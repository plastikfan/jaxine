    ...
    if (R.hasPath(['descendants', 'by'], spec)) {
      if (R.pathEq(['descendants', 'by'], 'index', spec)) {
        element[spec.labels.descendants] = R.indexBy(R.prop(spec.descendants.attribute),
          element[spec.labels.descendants]);
 ...
 */
const validateSpec = R.memoizeWith(R.identity, (spec) => {
  const throwIfPropNotIn = (propName, propValues, container) => {
    const val = R.prop(propName)(container);
    if (!R.includes(val, propValues)) {
      throw new Error(`"${propName}" not valid, must be in: "${propValues}": ${JSON.stringify(spec)}`);
    }
  };

  const throwIfMissing = (labelName, from, container) => {
    if (!R.has(labelName)(container)) {
      throw new Error(`"${labelName}" property missing from ${from}: ${JSON.stringify(spec)}`);
    }
  };
  throwIfMissing('attributesType', 'spec', spec);

  throwIfMissing('labels', 'spec', spec);
  const labels = R.prop('labels')(spec);

  throwIfMissing('element', 'spec.labels', labels);
  throwIfMissing('descendants', 'spec.labels', labels);
  throwIfMissing('text', 'spec.labels', labels);
