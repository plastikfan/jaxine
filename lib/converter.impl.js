...
function buildChildren (element, elementNode, spec, getOptions, previouslySeen) {
  ...
  if (selectionResult && selectionResult.length > 0) {
    ...
    if (R.hasPath(['descendants', 'by'], options)) {
      ...
      if (R.all(R.has(options.descendants.attribute))(children)) {
        if (R.pathEq(['descendants', 'by'], 'index', options)) {
          if (options.descendants.throwIfCollision) {
            ...
            //
            R.reduce((acc, val) => {
              if (R.contains(val[options.descendants.attribute], acc)) {
                throw new Error(`Element collision found: ${JSON.stringify(val)}`);
              }
              return R.append(val[options.descendants.attribute], acc);
            }, [])(descendants);
          }

          element[spec.labels.descendants] = R.indexBy(R.prop(options.descendants.attribute),
            descendants);
        } else if (R.pathEq(['descendants', 'by'], 'group', options)) {
          element[spec.labels.descendants] = R.groupBy(R.prop(options.descendants.attribute),
            descendants);
        }
      } else if (options.descendants.throwIfMissing) {
        const missing = R.find(R.not(R.has(options.descendants.attribute)))(children) || {};
        throw new Error(
          `Element is missing key attribute "${options.descendants.attribute}": (${JSON.stringify(missing)})`);
      }
    }
  }
