...
const fullSpecWithDefaults = Object.freeze({
  ...
  coercion: {
    attributes: {
      trim: true,
      matchers: { // MATCHER LIST IS NOT DEFAULT
 ...
 * @param {*} container
 */
function fetchCoercionOption (context, path, container) {
  ...
  const defaultLens = R.lensProp(leafSegment);
  ...
  const resultValue = R.defaultTo(viewItem(defaultLens,
    fullSpecWithDefaults.coercion.attributes))(viewItem(itemLens, container));
...
function validateMatchers (context, matchers) {
  if (matchers) {
    ...
    if (R.has('string')(matchers)) {
      const last = R.last(Object.getOwnPropertyNames(matchers)).toLowerCase();
      if (last !== 'string') {
