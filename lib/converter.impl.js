...
/**
 * @function: transformAssocValue
 * @description: coerces string to a primitive value. A sequence of primitive
 *    types can be defined for "valueType". The order of their definition is
 *    significant, as this is the same order that coercion to that particular
 *    type will be invoked.
 *
 * @param {String|Array} valueType: a single primitive type or an array of.
 * @param {String} context: 'attributes' or 'textNodes'
 * @param {Object} spec: Describes structure of the built JSON object and parse options.
 ...
 * "succeeded" flag to indicate if coercion was successful.
 */
function transformAssocValue (valueType, context, spec, assocValue) {
  let coercedValue = null;
  let succeeded = false;

  let valueTypes = valueType;

  if (R.is(String)(valueType)) {
    valueTypes = [valueType];
  }

  if (R.is(Array)(valueTypes)) {
    valueTypes.some((t) => {
    ...
    throw new Error(`Invalid associative value type: ${functify(valueTypes)}`);
  }
    ...
    if (openExpr.test(collectionValue) && closeExpr.test(collectionValue)) {
      ...
      if (collectionType === '[]') {
        ...
        value = arrayElements.map((item) => {
          ...
          return (itemResult.succeeded) ? itemResult.value : item; // ??? IS THIS A BAD RETURN?
        });
      } else {
        ...
        if (R.includes(R.toLower(collectionType), ['map', 'weakmap', 'object', '{}'])) {
          ...
          arrayElements = R.reduce((acc, collectionPair) => {
            ...
            if (elements.length === 2) {
              const coercedResult = transformAssocValue(assocValueType, context, spec, elements[1]);

              if (coercedResult.succeeded) {
                return R.append([elements[0], coercedResult.value])(acc);
              }
            } else {
