 ...
 */
function buildElement (elementNode, parentNode, getOptions, previouslySeen = []) {
  let element = buildLocalAttributes(elementNode);
  element['_'] = elementNode.nodeName;

  const options = getOptions(elementNode.nodeName);

  if (options.recurse !== '') {
    element = recurseThroughAttribute(element, elementNode, parentNode, getOptions, previouslySeen);
  }
  ...
  if (elementNode.hasChildNodes()) {
    element = buildChildren(element, elementNode, getOptions, previouslySeen);
  }
 ...
 *    the rightmost attribute takes precedence.
 *
 ...
 */
function recurseThroughAttribute (element, elementNode, parentNode, getOptions, previouslySeen) {
  const options = getOptions(elementNode.nodeName);

  let { id, recurse } = options;
  ...
  if (recurse && recurse !== null) {
    ...
    // First get the inherited element(s), which is a csv inside @inherits.
    //
    const inheritsAttr = elementNode.getAttribute(recurse) || '';
    ...
    if (inheritsAttr && inheritsAttr !== '') {
      ...
      //
      const inheritsAttributes = R.split(',', inheritsAttr);

      if (inheritsAttributes.length > 1) {
        // Just need to map the at to a build element => array which we pass to merge
        //
        let inheritedElements = R.map(at => {
          ...
          //
          return buildElement(inheritedElementNode, parentNode, getOptions, previouslySeen);
        }, inheritsAttributes);
        ...
        let doMergeElements = (a, b) => {
          ...
          if (R.includes('_children', R.keys(a)) && R.includes('_children', R.keys(b))) {
        ...
        //
        const inheritedElementName = inheritsAttributes[0];
        ...
        //
        let inheritedElement = buildElement(inheritedElementNode, parentNode, getOptions, previouslySeen);
 ...
 * @param {*} name: The value of the "id" attribute.
 * @param {*} parentNode: The XML/XPath node which provides the context for the xpath
 *    expression.
 ...
 * @param {*} elementNode: The XML node being built
 * @param {callback(el:string)} getOptions: a function that returns the element builder
 *    options (@property:"id", @property:"recurse", @property: "discards").
 * @param {String[]} [previouslySeen=[]]: Used internally to guard against circular references.
 * @returns an object in generic format eg:
 ...
 */
function buildChildren (element, elementNode, getOptions, previouslySeen) {
  ...
  if (selectionResult && selectionResult.length > 0) {
    ...
    R.forEach(childElement => {
      // Now collate the children.
      //
      let child = buildElement(childElement, elementNode, getOptions, previouslySeen);
      children.push(child);
    }, elements);

    if (R.includes('_children', R.keys(element))) {
      let merged = R.concat(children, element['_children']);
  ...
  let text = composeText(elementNode);
