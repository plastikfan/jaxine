...
function createTypedCollection_ (t, arrayElements) {
  let collection;

  switch (t) {
    case 'int8array': collection = Int8Array.from(arrayElements);
      break;
    default:
      collection = Array.from(arrayElements);
  }
...
function execExpressionForOpenTag (open) {
  // !<Int8Array>[1,2,3,4] | !<[Int8Array]>[1,2,3,4]
  // to recognise sequences like "!<[]>[" or "!<Int8Array>[" which should
  // return "[]" and "Int8Array" respectively.
  //
  if (!execExpressionForOpenTag.typeRegExp) {
    /* eslint-disable-next-line no-useless-escape */
    execExpressionForOpenTag.typeRegExp = new RegExp(/<(?<type>[\w\[\]]+)>/);
  }

  let result = null;

  if (execExpressionForOpenTag.typeRegExp.test(open)) {
    const execResult = execExpressionForOpenTag.typeRegExp.exec(open);
...
// function extractTypeFromOpenTag (open) {
//   if (!extractTypeFromOpenTag.openTypes) {
//     extractTypeFromOpenTag.openTypes = R.memoizeWith(R.identity, execExpressionForOpenTag);
//   }

//   return extractTypeFromOpenTag.openTypes(open);
// }

function extractTypeFromCollectionValue (open) {
  return execExpressionForOpenTag(open);
}
...
function transformCollection (collectionValue, context, spec) {
  ...
  if (collectionType) {
    const openExpression = open.replace('<type>', ('<' + collectionType + '>'));
    ...
    if (openExpr.test(collectionValue) && closeExpr.test(collectionValue)) {
      ...
      const arrayElements = coreValue.split(delim);
      ...
      if (collectionType === '[]') {
        ...
        try {
          value = createTypedCollection_(R.toLower(collectionType), arrayElements);
        } catch (err) {
          value = arrayElements;
          succeeded = false;
        }
