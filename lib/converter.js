 ...
 *    - element: The name of the property that stores the XML element name.
 *    - descendants: The name of the property holding descendant XML elements structure.
 *    - text: The name of the property holding text local to the XML element as raw text or CDATA text.
 *    (typically set to "_text")
 *    - attribute (optional): The name of the property that stores the attributes array (if
 *    attributesType = "Array")
 *    (typically set to "_attributes")
 * @member attributesType: Determines how to hold attributes.Can be set to ("Member" | "Array")
 ...
 *    - "by": Determines how descendants are structured. By default,
 *    descendants will be stored as an array. Can be restructured into a map object where each
 *    descendant is keyed by the attribute.
 *    - "attribute": the name of the attribute to index/group by.
 *
 */
const Specs = {
  default: {
  ...
  indexById: { // id(name) attribute value must be unique
    labels: {
      element: '_',
      descendants: '_children',
      text: '_text',
  ...
  groupById: { // id(name) attribute does not have to be unique
    labels: {
      element: '_',
      descendants: '_children',
      text: '_text',
